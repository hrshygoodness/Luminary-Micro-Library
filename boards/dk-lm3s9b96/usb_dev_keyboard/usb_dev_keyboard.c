...
DefaultSpecialHandler(short sCol, short sRow, tBoolean bPress)
{
    ...
    g_eKeyboardState = STATE_SENDING;
    ulRetcode = USBDHIDKeyboardKeyStateChange((void *)&g_sKeyboardDevice,
                                g_ucModifiers, psKey[sCol].cUsageCode, bPress);
...
VirtualKeyboardKeyPress(short sCol, short sRow, tBoolean bPress)
{
    ...
    else
    {
        ...
        ulRetcode = USBDHIDKeyboardKeyStateChange(
                                             ...
                                             bPress);
        DEBUG_PRINT("Key \"%c\" %s\n",
                    psAlphaKeys->pcKey[g_eVirtualKeyState][sCol],
                    (bPress ? "pressed" : "released"));
...
main(void)
{
    tRectangle sRect;
    long lCenterX;
    unsigned long ulLastTickCount, ulProcessing;
    tBoolean bRetcode, bLastSuspend, bKeyPressed;

...
//
//*****************************************************************************
static volatile tBoolean g_bStartBootloader = false;
...
MainSetSpeed(void)
{
    ...
    if(g_sParameters.ucModulationType == MOD_TYPE_SENSORLESS)
    {
        if(g_sParameters.ulTargetSpeed < g_sParameters.ulSensorlessEndSpeed)
        {
            g_sParameters.ulTargetSpeed = g_sParameters.ulSensorlessEndSpeed;
        }
    }
...
MainUpdatePAdjI(long lNewPAdjI)
{
    ...
    else
    {
        ...
        g_lPowerIntegrator = ((g_lPowerIntegrator / lNewPAdjI) *
                                  g_sParameters.lPAdjI);
    }

    //
    // Save the new I coefficient.
    //
    g_sParameters.lPAdjI = lNewPAdjI;
    ...
    switch(g_ulStartupState)
    {
        ...
        case 5:
        {
            ...
            if(g_ulStateCount == 0)
            {
                ...
                {
                    g_ulAccelRate = g_sParameters.usAccelPower << 16;
                    g_ulDecelRate = g_sParameters.usDecelPower << 16;
                }
...
MainPowerController(void)
{
    ...
    // Perform the actual PI controller computation.
    //
    lTempP = MainLongMul(g_sParameters.lPAdjP, lError);
    lTempI = MainLongMul(g_sParameters.lPAdjI, g_lPowerIntegrator);
    lError = lTempP + lTempI;
...
//!
//! This function adjusts the motor drive power toward a given target
...
MainPowerHandler(unsigned long ulTarget)
{
    ...
    if(ulTarget > g_ulPower)
    {
        ...
        if(g_sMotorCurrent >= (g_sParameters.sAccelCurrent + 200))
        {
            ulNewValue = g_sParameters.usAccelPower * 128;
        }
        else
        {
            ulNewValue = g_sParameters.usAccelPower * 128 *
        ...
        // acceleration rate (i.e. the acceleration rate has been changed).
        //
        if(g_ulAccelRate > (g_sParameters.usAccelPower << 16))
        {
            //
            // Reduce the acceleration rate to the requested rate.
            //
        ...
        // acceleration rate.
        //
        else if(g_ulAccelRate < (g_sParameters.usAccelPower << 16))
        {
            //
            // Increase the acceleration rate by 15 RPM, slowly
            // returning it to the desired rate.
    ...
    else
    {
        ...
        if(g_ulBusVoltage > (g_sParameters.ulDecelV + 63))
        {
            ulNewValue = g_sParameters.usDecelPower * 1024;
        }
        else
        {
            ulNewValue = (g_sParameters.usDecelPower * 1024 *
        ...
        // deceleration rate (i.e. the deceleration rate has been changed).
        //
        if(g_ulDecelRate > (g_sParameters.usDecelPower << 16))
        {
            //
            // Reduce the deceleration rate to the requested rate.
            //
        ...
        // deceleration rate.
        //
        else if(g_ulDecelRate < (g_sParameters.usDecelPower << 16))
        {
            //
            // Increase the deceleration rate by 15 RPM, slowly
            // returning it to the desired rate.
    ...
    if(g_ulState & STATE_FLAG_RUN)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                // converted to 18.14 fixed-point format.
                //
                ulTarget = (g_sParameters.ulTargetPower << 14);
            }
        }

...
//!
//! Since an AC induction motor is not a synchronous machine, the speed of the
//! rotor does not match the speed of the drive.  Therefore, an input is
//! provided for connecting to either an optical encoder or a tachogenerator
//! from which the rotor speed can be determined.
//!
...
//!
//! When running at higher speeds, the number of edges in a fixed time period
//! are counted to determine the speed of the rotor (referred to as edge count
//! mode).  The velocity capture feature of the quadrature encoder module is
...
//
//*****************************************************************************
unsigned short g_usRotorFrequency = 0;

//*****************************************************************************
//
//! Updates the current rotor frequency.
//!
//! \param usFrequency is the newly measured frequency.
//!
//! This function takes a newly measured rotor frequency and uses it to update
//! the current rotor frequency.  If the new frequency is different from the
//! current frequency by too large a margin, the new frequency measurement is
//! discarded (a noise filter).  If the new frequency is accepted, it is passed
...
SpeedNewValue(unsigned short usFrequency)
{
    ...
    //
    // Compute the difference in the frequency.
    //
    if(usFrequency > g_usRotorFrequency)
    {
        usDelta = usFrequency - g_usRotorFrequency;
    }
    else
    {
        usDelta = g_usRotorFrequency - usFrequency;
    }

    //
    // If the frequency is too large then return without updating the rotor
    // frequency.
    //
    if(usDelta > 100)
    {
    ...
    //
    // Pass the new rotor frequency reading through the low pass filter.
    //
    g_usRotorFrequency = ((g_usRotorFrequency * 3) + usFrequency) / 4;
}
...
//! computed and used as a measure of the rotor frequency.
//!
//! \return None.
//
//*****************************************************************************
...
GPIOCIntHandler(void)
{
    ...
    // from forcing the rotor frequency to zero.
    //
    HWREGBITW(&g_ulSpeedFlags, FLAG_EDGE_BIT) = 1;

    //
    // Compute the new frequency from the time between edges.
    //
    SpeedNewValue((SYSTEM_CLOCK * 10) /
...
//! edge counting mode for rotor frequency determination, the number of edges
...
QEIIntHandler(void)
{
    ...
    if(HWREGBITW(&g_ulSpeedFlags, FLAG_COUNT_BIT) == 0)
    {
        ...
        {
            //
            // No edge was seen, so set the rotor frequency to zero.
            //
            g_usRotorFrequency = 0;

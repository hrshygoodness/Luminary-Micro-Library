...
main(void)
{
    ...
    GrStringDrawCentered(&g_sContext, "udma-demo", -1, ulCenterX, 4, 0);

    //
    // Show the clock frequency on the display.
    //
    GrContextFontSet(&g_sContext, g_pFontFixed6x8);
    usnprintf(cStrBuf, sizeof(cStrBuf), "%u MHz",
              ROM_SysCtlClockGet() / 1000000);
    GrStringDrawCentered(&g_sContext, cStrBuf, -1, ulCenterX, 14, 0);
    ...
    while(1)
    {
        ...
        if(g_ulSeconds != ulPrevSeconds)
        {
            ...
            //
            usnprintf(cStrBuf, sizeof(cStrBuf), "CPU %2u%%",
                      g_ulCPUUsage >> 16);
            GrStringDrawCentered(&g_sContext, cStrBuf, -1, ulCenterX, 44, 1);

            //
            // Tell the user how many seconds we have to go before ending.
            //
            usnprintf(cStrBuf, sizeof(cStrBuf), " %d seconds ",
                      10 - g_ulSeconds);
            GrStringDrawCentered(&g_sContext, cStrBuf, -1, ulCenterX, 54, 1);
            ...
            //
            usnprintf(cStrBuf, sizeof(cStrBuf), "%8u B/s ",
                      ulBytesTransferred);
            GrStringDraw(&g_sContext, cStrBuf, -1, 24, 22, 1);
            ...
            //
            usnprintf(cStrBuf, sizeof(cStrBuf), "%7u B/s ",
                      ulBytesTransferred);
            GrStringDraw(&g_sContext, cStrBuf, -1, 30, 32, 1);
        }

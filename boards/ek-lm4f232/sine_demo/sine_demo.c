...
//!
//! This example uses the floating point capabilities of the Stellaris
//! Cortex-M4 processor to compute a sine wave and show it on the display.
//
//*****************************************************************************
...
unsigned char g_pucOffscreenBuf[OFFSCREEN_BUF_SIZE];
...
StripChart(g_sStripChart, WIDGET_ROOT, 0, 0, &g_sCFAL96x64x16, 0, 0, 96, 64,
           ...
           &sAxisX, &sAxisY, &g_sOffscreenDisplay);
...
//
//*****************************************************************************
static signed char g_cSeriesData[SERIES_LENGTH];
...
main(void)
{
    ...
    //
    GrOffScreen4BPPInit(&g_sOffscreenDisplay, g_pucOffscreenBuf, 96, 64);
    ...
    //
    g_sSeries.pvData = g_cSeriesData;
    ...
    while(1)
    {
        ...
        {
            memmove(&g_cSeriesData[0], &g_cSeriesData[1], SERIES_LENGTH - 1);
        }
        ...
        else
        {
            ...
            // the data series.
            //
            g_sSeries.usNumItems = ulItemCount;
        }
        ...
        //
        fRadians = fElapsedTime * 2.0F * (float)M_PI;
        ...
        //
        g_cSeriesData[ulItemCount - 1] = fSine * 32.0F;

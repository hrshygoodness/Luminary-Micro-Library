...
DisplayStatus(tContext *psContext, char *pcStatus)
{
    ...
    rectLine.sYMin = GrContextDpyHeightGet(psContext) - (TEXT_HEIGHT + 2);
    rectLine.sYMax = rectLine.sYMin + GrFontHeightGet(TEXT_FONT) + 1;
    ...
    GrStringDrawCentered(psContext, pcStatus, -1,
                         ...
                         rectLine.sYMin + (TEXT_HEIGHT / 2), false);
}

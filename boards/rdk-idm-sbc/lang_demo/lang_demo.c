...
//! the boot loader to allow a new application image to be downloaded.  If using
//! a recent version of LMFlash, firmware download can also be initiated
//! automatically via a ``magic packet'' sent from LMFlash to the IDM-SBC.  When
//! this packet is received, the application transfers control to the boot
//! loader without any required user interaction.
//!
...
//******************************************************************************
//
// The DMA control structure table required by the sound driver.
//
//******************************************************************************
#ifdef ewarm
...
//
//*****************************************************************************
Canvas(g_sFirmwareUpdate, g_psPanels + 2, 0, 0, &g_sKitronix320x240x16_SSD2119,
       0, 26, 320, 166, CANVAS_STYLE_APP_DRAWN, 0, 0, 0, 0, 0, 0,
       OnFirmwarePaint);
...
RectangularButton(g_sPrevious, 0, 0, 0, &g_sKitronix320x240x16_SSD2119, 0, 190,
                  ...
                  FONT_20PT, g_pcMinus, g_pucBlue50x50, g_pucBlue50x50Press,
                  0, 0, OnPrevious);
...
OnFirmwareUpdate(tWidget *pWidget)
{
    ...
    //
    // Change the button text to show that an update is starting.
    //
...
main(void)
{
    ...
    //
    // Configure and enable uDMA since this is required by the sound driver.
    //
    ...
    LocatorAppTitleSet("RDK-IDM-SBC lang_demo");
    ...
    //
    // Adjust the rectangle then fill the center with blue.
    //
    sRect.sXMin++;
    sRect.sYMin++;
    sRect.sXMax--;

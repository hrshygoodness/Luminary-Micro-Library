...
PopulateFileListBox(tBoolean bRepaint)
{
    ...
    //
    // Start by inserting at the top of the list box.
    //
    ulWavCount = 0;
    ...
    while(1)
    {
        ...
        //
        if((ulWavCount < NUM_LIST_STRINGS) &&
           ...
           (ustrstr(g_sFileInfo.fname, ".WAV"))))
        {
            ...
            {
                usnprintf(g_pcFilenames[ulWavCount], MAX_FILENAME_STRING_LEN,
                          "0:/%s", g_sFileInfo.fname);
                ListBoxTextAdd(&g_sDirList, g_pcFilenames[ulWavCount]);

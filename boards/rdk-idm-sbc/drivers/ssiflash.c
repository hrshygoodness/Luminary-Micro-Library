//*****************************************************************************
//
// ssiflash.c - Driver for the Winbond Serial Flash on the rdk-idm-sbc board.
//
...
SSIFlashStatusGet(void)
{
    ...
    //
    ROM_GPIOPinWrite(SFLASH_CS_BASE, SFLASH_CS_PIN, 0);

    //
    // Send the status register read instruction and read back a dummy byte.
    //
    ROM_SSIDataPut(SFLASH_SSI_BASE, INSTR_READ_STATUS);
    ROM_SSIDataGet(SFLASH_SSI_BASE, &ulStatus);

    //
    // Write a dummy byte then read back the actual status.
    //
    ROM_SSIDataPut(SFLASH_SSI_BASE, 0xFF);
    ROM_SSIDataGet(SFLASH_SSI_BASE, &ulStatus);

    //
    // Deassert the chip select for the serial flash.
    //
    ROM_GPIOPinWrite(SFLASH_CS_BASE, SFLASH_CS_PIN, SFLASH_CS_PIN);
...
SSIFlashRxFlush(void)
{
    ...
    while(ROM_SSIDataGetNonBlocking(SFLASH_SSI_BASE, &ulDummy))
    {
        //
                         ...
                         unsigned short usLen)
{
    ...
    //
    ROM_GPIOPinWrite(SDCARD_CS_BASE, SDCARD_CS_PIN, SDCARD_CS_PIN);
    ...
    // transaction.
    //
    ROM_SSIDataPut(SFLASH_SSI_BASE, ucInstruction);
    ROM_SSIDataGet(SFLASH_SSI_BASE, &ulDummy);
    ...
    {
        ROM_SSIDataPut(SFLASH_SSI_BASE, ucData[ulLoop]);
        ROM_SSIDataGet(SFLASH_SSI_BASE, &ulDummy);
    }
}
...
SSIFlashInstructionRead(unsigned char *ucData, unsigned long ulLen)
{
    ...
    for(ulLoop = 0; ulLoop < ulLen; ulLoop++)
    {
        ...
        //
        ROM_SSIDataGet(SFLASH_SSI_BASE, &ulData); /* read data frm rx fifo */
...
//! rdk-idm-sbc board.  Two independent GPIOs are used to provide chip selects
//! for these two devices but care must be taken when using both in a single
//! application, especially during intialization of the SDCard when the SSI
...
SSIFlashInit(void)
{
    ...
    //
    ROM_SysCtlPeripheralEnable(SFLASH_SSI_PERIPH);
    ROM_SysCtlPeripheralEnable(SFLASH_SSI_GPIO_PERIPH);
    ROM_SysCtlPeripheralEnable(SFLASH_CS_PERIPH);
    ROM_SysCtlPeripheralEnable(SDCARD_CS_PERIPH);
    ...
    //
    ROM_GPIOPinTypeSSI(SFLASH_SSI_GPIO_BASE, SFLASH_SSI_PINS);
    ROM_GPIOPadConfigSet(SFLASH_SSI_GPIO_BASE, SFLASH_SSI_PINS, GPIO_STRENGTH_4MA,
                         GPIO_PIN_TYPE_STD_WPU);
    ROM_GPIOPinTypeGPIOOutput(SFLASH_CS_BASE, SFLASH_CS_PIN);
    ROM_GPIOPinTypeGPIOOutput(SDCARD_CS_BASE, SDCARD_CS_PIN);
    ...
    //
    ROM_SSIConfigSetExpClk(SFLASH_SSI_BASE, SysCtlClockGet(),
                           SSI_FRF_MOTO_MODE_0, SSI_MODE_MASTER, 10000000, 8);
    ROM_SSIEnable(SFLASH_SSI_BASE);
...
//!
//! This function returns the size of an eraseable block for the serial flash

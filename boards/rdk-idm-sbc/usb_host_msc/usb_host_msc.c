...
Canvas(g_sBackground, WIDGET_ROOT, &g_sUpBackground, &g_sPWD,
       &g_sKitronix320x240x16_SSD2119, 10, 60, 120, 170,
       CANVAS_STYLE_FILL, ClrBlack, 0, 0, 0, 0, 0, 0);
...
Canvas(g_sMACAddr, &g_sHeading, 0, 0,
...
Canvas(g_sIPAddr, &g_sHeading, &g_sMACAddr, 0,
...
//
//*****************************************************************************
Canvas(g_sHeading, WIDGET_ROOT, &g_sBackground, &g_sIPAddr,
...
TCPIPStackInit(void)
{
    ...
    LocatorAppTitleSet("RDK-IDM-SBC usb_host_msc");
...
{
    int nStatus;
    unsigned long ulIPAddr, ulDriveTimeout;
    ...
    while(1)
    {
        ...
        //
        // Is there a command waiting to be processed?  Keep looking until there
        // is or a remote firmware update request is received.
        //
        while(!g_bFirmwareUpdate && (UARTPeek('\r') < 0))
        {
            ...
            //
            // Do we need to do anything to do anything as a result of a USB
            // state change?
            //
            switch(g_eState)
            {
                case STATE_DEVICE_ENUM:
                {
                    ...
                    if(!PopulateFileListBox(true))
                    {
                        ...
                        //
                        // Reset the retry count so that it is ready for the
        ...
        //
        // If we received a remote firmware update request, exit the loop
        // immediately.
        //

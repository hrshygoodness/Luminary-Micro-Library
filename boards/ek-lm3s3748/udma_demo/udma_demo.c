...
//*****************************************************************************
//
// The DMA channel to use for memory transfers.  By default, use the "software"
// channel.
//
//*****************************************************************************
static unsigned long g_ulMemXferChannel = UDMA_CHANNEL_SW;
...
uDMAIntHandler(void)
{
    ...
    //
    ulMode = uDMAChannelModeGet(g_ulMemXferChannel);
    if(ulMode == UDMA_MODE_STOP)
    {
        ...
        //
        uDMAChannelTransferSet(g_ulMemXferChannel, UDMA_MODE_AUTO,
                               g_ulSrcBuf, g_ulDstBuf, MEM_BUFFER_SIZE);

        //
        // Initiate another transfer.
        //
        uDMAChannelEnable(g_ulMemXferChannel);
        uDMAChannelRequest(g_ulMemXferChannel);
    }
...
InitSWTransfer(void)
{
    ...
    //
    uDMAChannelAttributeDisable(g_ulMemXferChannel,
    ...
    // are higher priority.
    //
    uDMAChannelControlSet(g_ulMemXferChannel | UDMA_PRI_SELECT,
    ...
    //
    uDMAChannelTransferSet(g_ulMemXferChannel | UDMA_PRI_SELECT,
...
main(void)
{
    ...
    //
    // Determine which DMA channel to use for memory transfers.
    // Due to chip erratum, the "software" DMA channel may not correctly
    // generate an interrupt at the completion of the transfer.  Check to
    // see if a work around is needed.
    //
    if(!(HWREG(SYSCTL_DC7) & SYSCTL_DC7_DMACH30))
    {
        //
        // If the software channel does not work correctly, then the work
        // around is to use an unassigned channel.  This part does not have
        // an I2S peripheral, so the I2S DMA channel can be used.
        //
        g_ulMemXferChannel = UDMA_CHANNEL_I2S0RX;
    }

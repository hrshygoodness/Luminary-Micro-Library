//*****************************************************************************
//
// class-d.c - Audio driver for the Class-D amplifier.
//
...
ClassDPWMHandler(void)
{
    ...
    //
    ROM_PWMGenIntClear(PWM0_BASE, PWM_GEN_1, PWM_INT_CNT_ZERO);
    ...
    if(HWREGBITW(&g_ulClassDFlags, CLASSD_FLAG_STARTUP))
    {
        ...
        //
        ROM_PWMDeadBandEnable(PWM0_BASE, PWM_GEN_1, 0, g_ulClassDStep);
    ...
    if(HWREGBITW(&g_ulClassDFlags, CLASSD_FLAG_SHUTDOWN))
    {
        ...
        if(g_ulClassDStep == (g_ulClassDPeriod - 2))
        {
            ...
            //
            ROM_PWMOutputState(PWM0_BASE, PWM_OUT_2_BIT | PWM_OUT_3_BIT,
                               false);
    ...
    ROM_PWMPulseWidthSet(PWM0_BASE, PWM_OUT_2, lDutyCycle);
...
ClassDInit(unsigned long ulPWMClock)
{
    ...
    //
    // Set GPIO F2 and F3 as PWM pins.  They are used to output the PWM2 and
    // PWM3 signals.
    //
    ...
    ROM_PWMGenPeriodSet(PWM0_BASE, PWM_GEN_1, g_ulClassDPeriod);

    //
    // Start the PWM outputs with one clock long pulses.
    //
    ROM_PWMPulseWidthSet(PWM0_BASE, PWM_OUT_2, 1);
    ROM_PWMDeadBandEnable(PWM0_BASE, PWM_GEN_1, 0, g_ulClassDPeriod - 2);

    //
    // Disable the PWM2 and PWM3 output signals.
    //
    ROM_PWMOutputState(PWM0_BASE, PWM_OUT_2_BIT | PWM_OUT_3_BIT, false);
    ...
    //
    ROM_PWMGenIntTrigEnable(PWM0_BASE, PWM_GEN_1, PWM_INT_CNT_ZERO);

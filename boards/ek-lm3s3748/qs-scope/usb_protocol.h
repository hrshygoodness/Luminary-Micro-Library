...
// and optional command or status-specific data.  Each packet sent is,
// therefore, a tScopePacket structure with optional data appended to the end
// of it.
//
...
// to the host.  As for host to device commands, the ucVersion field must be
// the protocol version number (SCOPE_PROTOCOL_VERSION_1 currently) and
// ucHdrLength must be set to sizeof(tScopePacket) which will equal 12 assuming
// your compiler packing macros are correctly configured.
//
// ucPacketType                ucParam          ulParam        ulDataLength
// ------------                -------          -------        ------------
...
// is usually accomplished using the PACKED modifier macros but, for IAR
...
//
//*****************************************************************************
#define SCOPE_PROTOCOL_VERSION_1 \
                                0x01
...
typedef struct
{
    ...
    // zero, usSample1mVolts contains a sample for channel 1 of the
    // oscilloscope and usSample2mVolts contains a channel 2 sample.  If
    // non-zero, the channels are swapped with usSample1mVolts containing a
    // channel 2 sample and usSample2mVolts containing a channel 1 sample.
    //
...
//
//*****************************************************************************
#define SCOPE_TRIGGER_TYPE_LEVEL \
                                0x00000000
#define SCOPE_TRIGGER_TYPE_RISING \
                                0x00000001
#define SCOPE_TRIGGER_TYPE_FALLING \
                                0x00000002
#define SCOPE_TRIGGER_TYPE_ALWAYS \
                                0x00000003

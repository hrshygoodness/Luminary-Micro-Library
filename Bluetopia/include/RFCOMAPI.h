...
BTPSAPI_DECLARATION int BTPSAPI RFCOMM_Get_Data_Queuing_Parameters(unsigned int BluetoothStackID, unsigned int *MaximumNumberDataPackets, unsigned int *QueuedDataPacketsThreshold);
   ...
   /* will return zero.  If an error occurs while registering the       */
   /* parameters, a negative value will be returned that identifies the */
...
BTPSAPI_DECLARATION int BTPSAPI RFCOMM_Register_Server_Channel(unsigned int BluetoothStackID, Byte_t ServerChannel, RFCOMM_Event_Callback_t RFCOMM_Event_Callback, unsigned long CallbackParameter);
   ...
   /* will fail.  The ServerChannel parameter specifies the channel that*/
   /* is to be unregistered.  If the Server information is successfully */
   ...
   /* returned that identifies the reason for the failure.              */
BTPSAPI_DECLARATION int BTPSAPI RFCOMM_Un_Register_Server_Channel(unsigned int BluetoothStackID, Byte_t ServerChannel);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_RFCOMM_Un_Register_Server_Channel_t)(unsigned int BluetoothStackID, Byte_t ServerChannel);
#endif
